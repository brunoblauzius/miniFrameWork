...
function modalAlert(options)
{
    ...
    html += '               <p>'+message+'</p>';
    if(button!=false){
        html += '               <div class="text-center" style="max-width:80px; margin:auto;"><button type="button" class="btn-yellow btn-'+style+'" data-dismiss="modal">'+button+'</button></div>';
    }
    html += '           </div>';
    html += '           <div class="line-border-all '+style+'"></div>';
    html += '       </div>';
    html += '   </div>';
    html += '</div>';
