    ...
    function min_loader( este ){
        var elemento = '<div class="row" id="min-loader"><div class="" >'+
                            '<div id="loader" class="loader-35"></div>'+
                        '</div></div>';
        $( elemento ).appendTo( este );
    }
    ...
    function trataErros( erros, form ) {
        
            $('.alert-error').remove();
            $('#'+form).find('div').removeClass('has-error');
            $.each(erros,function( key, value ) {
                 $item = $('#'+form+' input[name$="['+key+']"], textarea[name$="['+key+']"], select[name$="['+key+']"]').parent('.form-group, .pdd-v2');
                 $('#'+form+' input[name$="['+key+']"], textarea[name$="['+key+']"], select[name$="['+key+']"]').parents('.form-group, .pdd-v2').addClass('has-error');
                 //$('<small class="control-label alert-error" for="inputError"><i class="fa fa-times-circle-o"></i> '+value+' </small>').appendTo( $item ); 
            });  
    }
    ...
    function tratarJSON( data ) {
        ...
        if( data.erros ){
            trataErros(data.erros, data.form);
        }
        if( data.funcao ){
            eval(data.funcao);
        }
...
function infoErro( mensagem, elementoDiv ) {
        ...
        elemento = '<div class="row msgError"><div class="col-md-12">'
                           ...
                           + '<div class="alert alert-danger">'
                             //+   '<h4>Erro: </h4>'
                             +   "<p style=''> NOTIFICAÇÃO: " +  mensagem  +  '</p>'
                            +'</div><!-- /.box -->'
                        +'</div></div><!-- /.col -->';
        //( elemento ).prependTo( elementoDiv ).delay(5000).fadeOut();
        $( elemento ).appendTo( elementoDiv ).delay(5000).fadeOut(500);
        $('.row input, select , textarea').removeClass('has-error');
    }
